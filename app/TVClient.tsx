...
function TVDisplay({
  ...
  const loadData = async (mId: string) => {
    try {
      ...
      const { data: j } = await supabase
        ...
        .limit(3);
      if (alive.current) {
        const sorted = ((j ?? []) as JumuahRow[]).sort((a, b) => a.slot - b.slot);
        const deduped = sorted.filter(
          (slot, index, arr) => index === arr.findIndex((x) => x.slot === slot.slot)
        );

        if (deduped.length > 0) {
          setJumuahSlots(deduped.slice(0, 3));
        } else {
  ...
  const minutesToJamaat = nextPrayerRow
    ...
    : null;
  const isQuietMode =
    minutesToJamaat != null && minutesToJamaat >= 0 && minutesToJamaat <= 5;
